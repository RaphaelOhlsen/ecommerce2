...
public class AvaliacaoProdutoController {
    ...
    public AvaliacaoProduto salvarAvaliacaoProduto(@RequestBody @Valid AvaliacaoProduto avaliacaoProduto) throws ExceptionEcommerce {
        ...
        pessoaRepository.findById(avaliacaoProduto.getEmpresa().getId()).
                orElseThrow(() -> new ExceptionEcommerce("Empresa não encontrada"));

        produtoRepository.findById(avaliacaoProduto.getProduto().getId()).
                orElseThrow(() -> new ExceptionEcommerce("Produto não encontrado"));

        pessoaFisicaRepository.findById(avaliacaoProduto.getPessoa().getId()).
                orElseThrow(() -> new ExceptionEcommerce("Pessoa não encontrada"));
    ...
    public ResponseEntity<String> deletarAvaliacaoProduto(@PathVariable("id") Long id) throws ExceptionEcommerce {
        avaliacaoProdutoRepository.findById(id).
                orElseThrow(() -> new ExceptionEcommerce("Avaliação não encontrada"));
    ...
    public ResponseEntity<List<AvaliacaoProduto>> obterAvaliacoesPorProduto(@PathVariable("idProduto") Long idProduto) throws ExceptionEcommerce {
    ...
    public ResponseEntity<List<AvaliacaoProduto>> obterAvaliacoesPorProdutoPessoa(
            @RequestParam("idProduto") Long idProduto, @RequestParam("idPessoa") Long idPessoa) throws ExceptionEcommerce {
        produtoRepository.findById(idProduto).
                orElseThrow(() -> new ExceptionEcommerce("Produto não encontrado"));

        pessoaFisicaRepository.findById(idPessoa).
                orElseThrow(() -> new ExceptionEcommerce("Pessoa não encontrada"));
